...
if( !defined( "PHP_SIMPLE_XLS_GEN" ) ) {
   ...
   class  PhpSimpleXlsGen {
     ...
     function Header( $text="" ) {
        if ( $text == "" ) {
           $text = "Generated by Order Master 2 at ".date("r");
        }
        if ( $this->totalcol < 1 ) {
          $this->totalcol = 1;
        }
     ...
     function SendFile( $filename)
     {
        ...
        if (defined( "THEXLS_FILE")) {
            
            $fn = 'userfiles/'.str_replace ('.xls','_'.H::random_string ('lower',5).'.xls',$filename);
            
            $fp = fopen ('./'.$fn,'w');
            fwrite ($fp,$this->xls_data);
            fclose ($fp);
            
            header ("Location: ".Y::bu().$fn);
